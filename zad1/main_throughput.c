...
void sender_sync_send(long message_size, long repetitions, long delta, long max)
{
	...
	printf("Message Size [bytes], Time Taken [s], Time Taken [ms], Time Taken [us], Time Taken [ns], Throughput [Mbit/s]\n");
	for (; message_size <= max; message_size *= delta)
	{
		...
		double throughput_mbits = message_size / avg_s / 1024 / 128;
		printf("%d,\t\t%f,\t\t%f,\t\t%f,\t\t%f,\t\t%f\n", message_size, avg_s, avg_s * 1e3, avg_s * 1e6, avg_s * 1e9, throughput_mbits);
	}
}
